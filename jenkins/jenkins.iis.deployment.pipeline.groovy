pipeline {
    ...
    stages {
        ...
        stage("Stop Webservices") {
            steps{
                bat("iisreset ${env.serverWebSvc} /stop")
            }
        }
        stage("Publish Webservice"){
            steps{
                sleep(time: 1, unit:"SECONDS")
                bat("${env.msbuild} Source/WebService/WebService.csproj /p:DeployOnBuild=true /p:PublishProfile=${env.publishProfile}-Default /p:Configuration=${env.configuration};Platform=\"AnyCPU\" /p:AllowUntrustedCertificate=true /p:Username=${env.serverUser} /p:Password=${env.serverPass}")
            }
        }
        stage("Database"){
            steps {
        ...
        stage("Start Webservices"){
            steps{
                bat("iisreset ${env.serverWebSvc} /start")
            }
        }
    }    
